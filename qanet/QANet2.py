...
class Embedding(nn.Module):
    ...
    def forward(self,wd_emb):
        ...
        wd_emb = wd_emb.transpose(1, 2)#(batch_size,dim,length)
        emb = self.high(wd_emb)
        return emb
'''
经过Embedding后，整个张量的shape==(batch_size,dim,seq_length)
'''
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask):
        bs, _, l_x = x.size()
        x = x.transpose(1,2)#batch_size,seq_lengh,dim
...
class Model(nn.Module):
    def __init__(self, word_mat):
        ...
        self.question_conv = DepthwiseSeparableConv(d_word,d_model, 5)#5代表kernel_size
        ...
        self.model_enc_blks = nn.ModuleList([enc_blk] * 5)
        self.out = Pointer()
    ...
    def forward(self, Cwid,  Qwid):
        ...
        #print("After word embedding : ",Cw.size(),Qw.size())
        C, Q = self.emb(Cw), self.emb(Qw)
        #print("After Embedding : ",C.size(),Q.size())
        #(batch_size,dim,length)
        C = self.context_conv(C)  
        Q = self.question_conv(Q)  
        #print("After context conv : ",C.size(),Q.size())
